...
import { fundEscrow } from '@cuboid/api-sdk';
...
export async function POST(req: Request) {
  try {
    ...
    const result = await fundEscrow(body.id, body.actorId);
    ...
    return NextResponse.json({ success: false, errorCode: 'ESCROW_FUND_FAILED', message: (err as Error).message, requestId: `req_${Date.now()}`, timestamp: new Date().toISOString() }, { status: 500 });
  }
}
