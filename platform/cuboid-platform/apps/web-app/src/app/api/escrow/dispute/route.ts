...
import { disputeEscrow } from '@cuboid/api-sdk';
...
export async function POST(req: Request) {
  try {
    ...
    const result = await disputeEscrow(body.id, body.actorId);
    ...
    return NextResponse.json({ success: false, errorCode: 'ESCROW_DISPUTE_FAILED', message: (err as Error).message, requestId: `req_${Date.now()}`, timestamp: new Date().toISOString() }, { status: 500 });
  }
}
