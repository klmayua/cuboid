...
import { listMyEscrows } from '@cuboid/api-sdk';
...
export async function GET(req: Request) {
  try {
    ...
    const result = await listMyEscrows(orgId);
    return NextResponse.json({ success: true, data: result, requestId: `req_${Date.now()}`, timestamp: new Date().toISOString() });
  } catch (err) {
    return NextResponse.json({ success: false, errorCode: 'ESCROW_LIST_FAILED', message: (err as Error).message, requestId: `req_${Date.now()}`, timestamp: new Date().toISOString() }, { status: 500 });
  }
}
