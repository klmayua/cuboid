...
import { verifySettlement } from '@cuboid/api-sdk';
...
export async function POST(req: Request) {
  try {
    ...
    const result = await verifySettlement(body.id, body.actorId);
    ...
    return NextResponse.json({ success: false, errorCode: 'SETTLEMENT_VERIFY_FAILED', message: (err as Error).message, requestId: `req_${Date.now()}`, timestamp: new Date().toISOString() }, { status: 500 });
  }
}
